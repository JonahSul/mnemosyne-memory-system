...
export interface PersistentTierItem {
	...
	// Tier-specific properties (KV optimized)
	...
	// KV-specific persistence tracking
	kvKey: string;
	vectorId?: string;
	persistenceLevel: 'kv_only' | 'kv_vector' | 'critical_protected';
	ttlSeconds?: number; // For short-term tier with auto-expiry
}
 ...
 */
export const PERSISTENT_TIER_CONFIG: Record<string, TierLimits> = {
	axiom: {
		maxItems: 100,
...
export class PersistentTier {
	 ...
	 */
	async store(item: Omit<PersistentTierItem, 'id' | 'timestamp' | 'kvKey' | 'tier' | 'persistenceLevel'>): Promise<string> {
		const id = `${this.tierName}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
		const timestamp = new Date().toISOString();
		const kvKey = `${this.keyPrefix}${id}`;

		const persistentItem: PersistentTierItem = {
			...item,
			id,
			timestamp,
			kvKey,
			tier: this.tierName as any,
	 ...
	 */
	async get(id: string): Promise<PersistentTierItem | null> {
		const kvKey = `${this.keyPrefix}${id}`;
		
		try {
			const kvData = await this.kv.get(kvKey);
			if (kvData) {
				const item = JSON.parse(kvData) as PersistentTierItem;
	 ...
	 */
	async search(query: string, limit: number = 10): Promise<PersistentTierItem[]> {
		const results: PersistentTierItem[] = [];
		...
		try {
			...
			if (indexData) {
				const itemIds: string[] = JSON.parse(indexData);
			...
			if (results.length < limit && this.config.persistenceLevel !== 'kv_only') {
				try {
					...
					for (const vResult of vectorResults) {
						const id = vResult.metadata?.id as string;
	 ...
	 */
	async listAll(limit: number = 100): Promise<PersistentTierItem[]> {
		try {
			const indexKey = `${this.keyPrefix}index`;
			const indexData = await this.kv.get(indexKey);
			...
			const itemIds: string[] = JSON.parse(indexData);
			const items: PersistentTierItem[] = [];
	 ...
	 */
	async checkPromotion(id: string): Promise<boolean> {
		const item = await this.get(id);
	...
	async prune(): Promise<number> {
		...
		let sortedItems: PersistentTierItem[];
	 ...
	 * Update tier index for fast enumeration
	 */
	private async updateTierIndex(id: string): Promise<void> {
		try {
			const indexKey = `${this.keyPrefix}index`;
	 ...
	 */
	private async removeTierIndex(id: string): Promise<void> {
		try {
			const indexKey = `${this.keyPrefix}index`;
			const existing = await this.kv.get(indexKey);
			
			if (existing) {
				let index: string[] = JSON.parse(existing);
...
export class PersistentTierMemorySystem {
	private tiers: Map<string, PersistentTier> = new Map();
	private storage: TierStorageConfig;
	 ...
	 */
	async search(query: string, limit: number = 10): Promise<PersistentTierItem[]> {
		const allResults: (PersistentTierItem & { tierBoost: number })[] = [];
		...
		allResults.sort((a, b) => 
			(b.importance * b.tierBoost) - (a.importance * a.tierBoost)
		);
	 ...
	 */
	async processPromotions(): Promise<number> {
	 ...
	 * Prune specific tier
	 */
	private async pruneTier(tierName: string): Promise<void> {
	 ...
	 */
	private determineTier(importance: number): string {
	...
	async exportAll(): Promise<{
		...
		const tierData: Record<string, PersistentTierItem[]> = {};
