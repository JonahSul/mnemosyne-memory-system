...
export async function applyFoundationMigrationLearningMode(memory) {
    // Apply core rules
    foundationMigrationLearningMode.coreRules.forEach((rule) => {
    ...
    foundationMigrationLearningMode.safetyConstraints.forEach((sc) => {
        memory.addBehavioralRule({
            id: sc.constraint,
            rule: sc.rationale,
    ...
    // Log migration
    const migrationId = await memory.logClaim(`Foundation migration ${foundationMigrationLearningMode.version} applied: Learning Mode crystallized`, { migration: foundationMigrationLearningMode.version }, 'foundation-migration', 'high');
