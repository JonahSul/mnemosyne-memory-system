...
export interface PrewarmingOperations {
	...
	createPrewarmingSessionStrategy(sessionContext: Record<string, unknown>): { targetConcepts: string[]; relatedTopics: string[]; priorityLevel: number; sessionId: string; context: Record<string, unknown> };
...
export class PrewarmingManager implements PrewarmingOperations {
	...
	createSessionPrewarmingStrategy(sessionContext: Record<string, unknown>) {
		...
		const targetConceptSet = new Set<string>();
		if (identifiedDomain) {
			identifiedDomain.split(/[^a-z0-9]+/).filter(Boolean).forEach(part => targetConceptSet.add(part));
		}

		for (const query of userQueries) {
			if (query.includes('database')) targetConceptSet.add('database');
			if (query.includes('sql')) targetConceptSet.add('sql');
			if (query.includes('performance')) targetConceptSet.add('performance');
			if (query.includes('tuning')) targetConceptSet.add('tuning');
		}
