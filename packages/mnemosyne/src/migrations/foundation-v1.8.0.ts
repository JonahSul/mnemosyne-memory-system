...
export interface TemporalMetadata {
    serverTimestamp: number; // Microsecond precision
    lamportClock: number;
...
export interface ExpansionStrategy {
    ...
    precisionCoefficient: number; // 0.0-1.0, higher = more precise/less expansive
    qualityValidation: boolean;
    generationTimestamp: string;
}

export interface SemanticAxes {
    nearSemanticNeighbor: SemanticAxis; // 0.85-0.95 similarity
    relatedConcept: ConceptualAxis;     // 0.60-0.80 similarity  
    analogicalPattern: AnalogicalAxis;  // 0.30-0.60 similarity
}
...
export interface ConceptualAxis extends SemanticAxis {
    conceptualDistance: number; // 0.0-1.0
}
...
export interface CausalityAnalysisResult {
    ...
    evidence: string[];
    analysisDetails: {
        lamportAnalysis: { relationship: string; confidence: number };
        vectorAnalysis: { relationship: string; confidence: number };
        hlcAnalysis: { relationship: string; confidence: number };
        consensus: { method: string; agreement: number };
    };
}
